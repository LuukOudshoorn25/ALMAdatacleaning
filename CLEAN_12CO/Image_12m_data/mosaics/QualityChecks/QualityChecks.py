...
def get_data():
	...
	os.system("cp -r /allegro1/allegro/data/projects/aGqnEW3z/analysis/oudshoorn/CLEAN_12CO/TotalPower/TP_12CO_regrid.image ./30Dor_TPalone.image")
	TP_image = "30Dor_TPalone.image"
	#importfits(fitsimage='',imagename='')

	importfits(fitsimage='../12+7m+TP+nomodel.fits',imagename="30Dor_12m_alone.image")
	IntAlone_image = "30Dor_12m_alone.image"

	importfits(fitsimage='../12+7m+TP+nomodel.feather.fits',imagename="30Dor_12+feather.image")
	Feather_image = "30Dor_12+feather.image"

	importfits(fitsimage='../12+7m+TP+model.fits',imagename="30Dor_12m+7m_model.image")
	Model_image = "30Dor_12m+7m_model.image"

	importfits(fitsimage='../12+7m+TP+model.feather.fits',imagename="30Dor_12m+7m_model_feather.image")
	Hybrid_image = "30Dor_12m+7m_model_feather.image"

	return TP_image, IntAlone_image, Feather_image, Model_image, Hybrid_image

TP_image, IntAlone_image, Feather_image, Model_image, Hybrid_image = get_data()

## Data: paths to images
## TP_image = "myTP.image"
## IntAlone_image = "ALMAalone.image"
## Feather_image = "ALMAfeather.image"
## Hybrid_image = "Hybrid.image"

## STEP 1a: Check axis
## Example:
#check_axis(ref_image=TP_image,target_im=[Model_image,Hybrid_image])

## STEP 1b: Drop image degeneracies (optional)
## Example:
drop_axis(ref_image=TP_image)
drop_axis(ref_image=IntAlone_image)
drop_axis(ref_image=Feather_image)
drop_axis(ref_image=Hybrid_image)
drop_axis(ref_image=Model_image)
## check again
#check_axis(ref_image=TP_image+"_subimage",target_im=[IntAlone_image+"_subimage",Feather_image+"_subimage"])
...
## Example:
get_IQA(ref_image = TP_image+"_subimage_masked",target_image=[IntAlone_image+"_subimage",Feather_image+"_subimage",Model_image+"_subimage",Hybrid_image+"_subimage"])

## STEP 3: Display IQA parameters
## (1) Accuracy parameter, (2) Fidelity, ...
## Example:
Compare_Apar_cubes(ref_image = TP_image+"_subimage_masked",target_image=[IntAlone_image+"_subimage",Feather_image+"_subimage",Model_image+"_subimage",Hybrid_image+"_subimage"])
